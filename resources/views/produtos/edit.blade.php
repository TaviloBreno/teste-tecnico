            ...
            <div class="bg-white shadow-xl rounded-lg p-8 space-y-6">
                ...
                <form method="POST" action="{{ route('produtos.update', $produto->id) }}">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="preco" id="preco" value="{{ old('preco', $produto->preco) }}"
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm px-4 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500" />
                        @error('preco')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="estoque" id="estoque" value="{{ old('estoque', $produto->estoque) }}"
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm px-4 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500" />
                        @error('estoque')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
